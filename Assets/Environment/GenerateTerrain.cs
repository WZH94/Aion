...
public class GenerateTerrain : MonoBehaviour
{
    ...
    [SerializeField]
    private float noiseScale_x = .05f;
    [SerializeField]
    private float noiseScale_y = .05f;

    [SerializeField]
    private float noiseScale_z = .05f;

    [SerializeField]
    private float noiseOffset_x = .05f;
    ...
    [SerializeField]
    private int grid_size_x, grid_size_y, grid_size_z = 20;
        ...
        for (int x = 0; x < grid_size_x; x++)
        {
            ...
            for (int y = 0; y < grid_size_y; y++)
            {
                for (int z = 0; z < grid_size_z; z++)
                {
                    ...
                    float noiseValue = Perlin3D(x * noiseScale_x + +noiseOffset_x, y * noiseScale_y, z * noiseScale_z);//get value of the noise at given x, y, and z.
                    ...
                    {//is noise value above the threshold for placing a block?
                        ...
                        blockMesh.transform.Rotate(rot, 0, 0);
                        
                        
                        float scaleRand = Random.Range(0.6f, 1.6f);
                        locScale = new Vector3(scaleRand, scaleRand, scaleRand);
                        blockMesh.transform.localScale = locScale;
...
#region Create Mesh
        ...
        float roty = 0f;//chunks rotation var
        ...
        {//for each list (of block data) in the list (of other lists)
            ...
            /*Vector3 target = new Vector3(200.0f, 0.0f, 0.0f);
            g.transform.RotateAround(target, Vector3.up, roty);*/
            block_count += 1;

            roty += (180 / 46);
            g.transform.RotateAround(new Vector3(100 ,0,0), new Vector3(0, 1, 1), roty); //rotate the chunks
            Debug.Log("number of blocks equals to:" + block_count);
            //transform.RotateAround (target.position, new Vector3 (0, 1, 0), 90 * Time.deltaTime);

            //"Assets/MaterialName.prefab"
        }
    ...
    private void Update()
    {
        ...
        /*if (deploy)
        {*/
        if (deploy) //generate the mesh when 'deploy' checked
        {
